...
-- thérapeute et une patiente, et vérifie ce que chacun atteint réellement à
...
values
  ...
  ('66666666-6666-6666-6666-666666666666', '00000000-0000-0000-0000-000000000000', 'authenticated', 'authenticated', 'patiente-a@test.local', '', now(), now(), now());
...
insert into public.patients (id, cabinet_id, auth_user_id, display_name, initials, program, subtitle, week_label, scale_label, scale_question) values
  ('55555555-5555-5555-5555-555555555555', '44444444-4444-4444-4444-444444444444', '66666666-6666-6666-6666-666666666666',
   'Patiente A', 'PA', 'Programme Liberté', 'Liberté · semaine 3 / 6', 'Semaine 3 sur 6', 'Envie de fumer', 'Où en est l''envie de fumer ?'),
...
begin
  ...
  select count(*) into n from public.therapy_sessions;
  if n <> 0 then raise exception 'FUITE : la patiente atteint le dossier clinique (% séance(s))', n; end if;
  select count(*) into n from public.journal_pages;
  if n <> 2 then raise exception 'La patiente devrait voir ses 2 pages, elle en voit %', n; end if;
  select count(*) into n from public.cabinets;
  if n <> 0 then raise exception 'FUITE : la patiente atteint % cabinet(s)', n; end if;

  perform set_config('role', 'none', true);
  raise notice 'Cloisonnement : revendeur, thérapeute et patiente conformes.';
end $$;
