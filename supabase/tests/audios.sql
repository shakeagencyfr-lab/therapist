-- ============================================================================
-- Les audios, sous les droits réels des patients (migration 0012).
--
--   praticienne → catalogue un audio, l'envoie à un patient
--   ce patient → lit le titre et la durée de son audio
--   un autre patient du même cabinet → ne voit rien de la bibliothèque
--
...
begin
  ...
  insert into auth.users (id, instance_id, aud, role, email, encrypted_password, email_confirmed_at, created_at, updated_at) values
    ...
    ('cccc0000-0000-4000-8000-00000000c012','00000000-0000-0000-0000-000000000000','authenticated','authenticated','test12-patient@exemple.fr','',now(),now(),now()),
  ...
  values (v_cab, 'Test P.', 'TP', '', '', '', '', '', 'test12-patient@exemple.fr') returning id into v_pat;
  insert into public.patients (cabinet_id, display_name, initials, program, subtitle, week_label, scale_label, scale_question, email)
  ...
  if t is distinct from 'Ancrage du soir' then raise exception 'ECHEC : le patient ne lit pas le titre de son audio (%)', t; end if;
  ...
  if n <> 0 then raise exception 'FUITE : un patient sans cet audio voit % ligne(s) de la bibliothèque', n; end if;

  raise exception 'REUSSITE : le patient lit le titre de l''audio qui lui est envoyé ; un autre patient du même cabinet ne voit rien de la bibliothèque. (Rien ne persiste.)';
end $$;
